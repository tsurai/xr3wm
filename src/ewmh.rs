...
pub fn get_active_window(xws: &XlibWindowSystem) -> Option<Window> {
    let root = xws.get_root_window();
    xws.get_property(root, "_NET_ACTIVE_WINDOW").map(|x| x[0])
}
...
pub fn set_desktop_viewport(xws: &XlibWindowSystem, workspaces: &[Workspace]) {
    ...
    let viewports: Vec<u64> = workspaces
        .iter()
        .map(|ws| {
            screens
                .get(ws.get_screen())
...
pub fn set_client_list(xws: &XlibWindowSystem, workspaces: &[Workspace]) {
    ...
    let clients: Vec<Window> = workspaces
        .iter()
        .map(|ws| ws.all())
...
pub fn set_wm_desktop(xws: &XlibWindowSystem, window: Window, idx: usize) {
    xws.change_property(
        window,
        "_NET_WM_DESKTOP",
        XA_CARDINAL,
...
pub fn set_wm_state(
    ...
    let active_atoms = xws
        .get_property(window, "_NET_WM_STATE")
        .unwrap_or_default();
    ...
    match mode {
        ...
        NET_WM_STATE_ADD | NET_WM_STATE_TOGGLE => {
            let (add_atoms, rem_atoms) = if mode == NET_WM_STATE_ADD {
                ...
                atoms
                    .iter()
                    .partition::<Vec<Atom>, _>(|x| !active_atoms.iter().any(|y| y == *x))
            };

            let atoms: Vec<Atom> = active_atoms
                .iter()
